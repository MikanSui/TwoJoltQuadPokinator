...
This spits out some stuff required by some firmware,
you're on your own, on this.

you need pycryptodomex for one.
'''
...
ctlr_CA_pubkey = RSA.construct((bytes_to_long(bytes([
...
class CtlrFlash:
    ...
    def __init__(s, path):
        s.path = path
        s.verify_fw()
        s.cert = CtlrCert(s.read_interleaved(0x5000, 0x800))
...
if __name__ == '__main__':
    filename = input('input filename of firmware: ')
    flash = CtlrFlash(f'./{filename}')
    cert = flash.cert
